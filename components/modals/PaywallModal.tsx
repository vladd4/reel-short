...
export default function PaywallModal({
  ...
  function startSubCheckout() {
    if (!selectedPlan) return
    setCheckoutType('subscription')
    setCheckoutStep(isLoggedIn ? 'payment' : 'signin')
  }

  function startCoinCheckout() {
    if (!selectedPack) return
    setCheckoutType('coins')
    setCheckoutStep(isLoggedIn ? 'payment' : 'signin')
  }
  ...
  const checkoutPlan =
    checkoutType === 'coins' && selectedPack
      ? {
          label: `${selectedPack.totalCoins.toLocaleString()} Coins`,
          price: `$${selectedPack.price}`,
        }
      : { label: selectedPlan?.name ?? '', price: `$${selectedPlan?.price ?? '0'}` }
  ...
  return (
    <>
      ...
      >
        <div
          className={`relative flex max-h-[92dvh] w-full flex-col overflow-hidden sm:mx-4 sm:max-w-2xl modal-drawer${visible ? ' open' : ''}`}
          style={{
            background: '#040405',
            boxShadow: '0 -8px 40px rgba(0,0,0,0.6), 0 0 0 1px rgba(255,255,255,0.04)',
          }}
        >
          <div className="relative flex flex-shrink-0 items-center justify-center pt-3 pb-1 sm:hidden">
            <div
          ...
          <div className="scrollbar-hide overflow-y-auto">
            <div className="space-y-6 px-5 pt-5 pb-6">
              {/* Subscription plans */}
              <div>
                <h2 className="mb-0.5 text-lg font-bold text-white">
                  VIP Unlock all series for free
                </h2>
                    ...
                    {plans.map((p) => {
                      ...
                      return (
                        <button
                          ...
                          style={{
                            background:
                              'linear-gradient(145deg, #f5e6c8 0%, #e8d09a 50%, #d4b870 100%)',
                        ...
                        >
                          <div
                            ...
                            style={{
                              background: 'radial-gradient(circle, #e3a119 0%, transparent 70%)',
                            }}
                          />
                          <p className="mb-2 text-sm font-bold" style={{ color: '#1a1205' }}>
                            {p.name}
                          </p>
                          ...
                          <div className="flex flex-wrap gap-2">
                            {p.features.map((f) => (
                              <span
                                ...
                                style={{ background: 'rgba(26,18,5,0.12)', color: '#3d2a05' }}
                              >
                                {f}
                              </span>
                            ))}
                  ...
                  {packsLoading ? (
                    ...
                    <>
                      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
                        {packs.map((pack) => (
                          <CoinCard
                            ...
                            onSelect={() =>
                              setSelectedPack((prev) => (prev?.id === pack.id ? null : pack))
                            }
                          />
                        ))}
                      </div>
                      ...
                      <button
                        ...
                        className="mt-4 w-full rounded-xl py-3.5 text-sm font-bold text-white transition-all hover:brightness-110 active:scale-95 disabled:cursor-not-allowed disabled:opacity-40"
                        style={{
                          background: selectedPack
                            ? 'linear-gradient(90deg, #2b009f 0%, #4500ff 100%)'
                            : 'rgba(255,255,255,0.08)',
                          boxShadow: selectedPack ? '0 4px 20px rgba(69,0,255,0.35)' : 'none',
                        }}
                      >
                        {selectedPack
                          ? `Buy ${selectedPack.totalCoins.toLocaleString()} Coins — $${selectedPack.price}`
                          : 'Select a coin pack'}
                      </button>
                    </>
                  )}
      ...
      {checkoutStep === 'payment' && (
        <CheckoutModal
          plan={checkoutPlan}
          packCode={checkoutType === 'coins' ? (selectedPack?.code ?? undefined) : undefined}
          planCode={checkoutType === 'subscription' ? (selectedPlan?.code ?? undefined) : undefined}
          onClose={() => setCheckoutStep(null)}
          onSubscribe={handleCheckoutComplete}
        />
      )}
    </>
  )
...
function CoinCard({
  ...
  return (
    <button
      ...
      style={{
        background: isSelected ? 'rgba(69,0,255,0.15)' : 'rgba(255,255,255,0.05)',
        border: `2px solid ${isSelected ? 'rgba(122,144,255,0.7)' : 'rgba(255,255,255,0.07)'}`,
        boxShadow: isSelected ? '0 0 0 1px rgba(122,144,255,0.3)' : 'none',
      }}
    >
      {pack.bonusPercent > 0 && (
        <span
