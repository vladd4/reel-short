...
import { EXCLUSIVE_OFFER_COINS, EXCLUSIVE_OFFER_PACK_CODE, EXCLUSIVE_OFFER_PRICE } from '@/constants'
...
type Step = 'signin' | 'payment' | null

const baseCoins = Math.round(EXCLUSIVE_OFFER_COINS / 1.1)
const bonusCoins = EXCLUSIVE_OFFER_COINS - baseCoins

const checkoutPlan = {
  label: `${EXCLUSIVE_OFFER_COINS} Coins`,
  price: EXCLUSIVE_OFFER_PRICE,
}

export default function ExclusiveOfferModal({ onClose, onPurchased }: Props) {
  useScrollLock()
  ...
  function handleClaim() {
    setStep(isLoggedIn ? 'payment' : 'signin')
  }
  ...
  return (
    <>
      ...
      >
        ...
        >
          ...
          >
            ...
            <button
              onClick={handleClaim}
              className="flex w-full cursor-pointer items-center justify-center gap-2 rounded-2xl py-4 text-base font-bold transition-all hover:brightness-110 active:scale-[0.98]"
              style={{
      ...
      {step === 'payment' && (
        <CheckoutModal
          plan={checkoutPlan}
          packCode={EXCLUSIVE_OFFER_PACK_CODE}
          onClose={() => setStep(null)}
          onSubscribe={handlePurchaseComplete}
        />
      )}
    </>
  )
